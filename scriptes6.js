...
let board;
let tile;
let isCurrentPlayerWon;
let isGameOver;
let num_of_pieces;
const humanDisk = 16773120;
const AIsDisk = 4095;
let isFirstMove;
let clickedColoumn;
...
function checkCurrentGameStatus() {
    if (checkHorizontalConnect(currentplayer) || checkVerticalConnection(currentplayer) || checkDiagnolConnection(currentplayer)) {
        isGameOver = true;
...
function findNextMove(checkForPlayer) {
    ...
    while (col < 7) {
        ...
        if (row.available) {
            ...
            const isFourConnectPossiable = checkHorizontalConnect(checkForPlayer) || checkVerticalConnection(checkForPlayer) || checkDiagnolConnection(checkForPlayer);
